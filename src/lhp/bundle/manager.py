...
class BundleManager:
    ...
    def __init__(
        ...
        )  # Default to base dir for compatibility
        ...
        if related_files:
            ...
            if self._is_lhp_generated_file(existing_file):
                if force and has_pipeline_config:
                    ...
                    self._create_new_resource_file(
    ...
    def generate_resource_file_content(
        ...
        Raises:
            LHPConfigError: If pipeline config validation fails (also a ValueError)
        """
        ...
        if catalog or schema:
            if not (catalog and schema):
                raise LHPConfigError(
                    category=ErrorCategory.CONFIG,
                    code_number="026",
                    title=f"Incomplete catalog/schema in pipeline '{pipeline_name}'",
                    details=(
                        f"Must define BOTH catalog AND schema, or neither, "
                        f"in pipeline '{pipeline_name}' config. "
                        f"Found: catalog={'defined' if catalog else 'missing'}, "
                        f"schema={'defined' if schema else 'missing'}"
                    ),
                    suggestions=[
                        "Define both 'catalog' and 'schema' in your pipeline_config.yaml",
                        "Or remove both fields to use default values from databricks.yml",
                        "Run 'lhp validate --env <env>' to check configuration",
                    ],
                    context={
            ...
            if not catalog.strip() or not schema.strip():
                raise LHPConfigError(
                    category=ErrorCategory.CONFIG,
                    code_number="026",
                    title=f"Empty catalog/schema after substitution in pipeline '{pipeline_name}'",
                    details=(
                        f"Pipeline '{pipeline_name}' config has empty catalog or schema "
                        f"after substitution token resolution."
                    ),
                    suggestions=[
                        "Check that your substitution file defines values for the catalog/schema tokens",
                        "Verify the substitution tokens in pipeline_config.yaml match your substitution file keys",
                        "Run 'lhp substitutions --env <env>' to inspect resolved tokens",
                    ],
                    context={
                        "Pipeline": pipeline_name,
                        "Catalog Value": repr(catalog),
    ...
    def _extract_first_global_catalog_schema(
    ...
    def _get_all_substitution_environments(self) -> List[str]:
        ...
        self.logger.debug(
            f"Found {len(environments)} substitution environments: {environments}"
        )
        return environments

    def _find_substitution_variables_for_values(
        self, catalog: str, schema: str, env: str
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            config = load_yaml_file(
                substitution_file, error_context="bundle substitution file"
            )
            ...
            if not env_config:
                self.logger.warning(
                    f"No configuration found for environment '{env}' in {substitution_file}"
                )
                return {"catalog_var": None, "schema_var": None}
            ...
            for var_name, var_value in env_config.items():
                if isinstance(var_value, str):
                    if var_value == catalog:
                        ...
                        self.logger.debug(
                            f"Found catalog variable: {var_name} = {var_value}"
                        )
                    if var_value == schema:
                        schema_var = var_name
                        self.logger.debug(
                            f"Found schema variable: {var_name} = {var_value}"
                        )
            ...
            self.logger.warning(
                f"Could not parse substitution file {substitution_file}: {e.title}"
            )
            return {"catalog_var": None, "schema_var": None}
        except Exception as e:
            self.logger.warning(
                f"Unexpected error reading substitution file {substitution_file}: {e}"
            )
            return {"catalog_var": None, "schema_var": None}
    ...
    def _validate_databricks_targets_exist(self, environments: List[str]) -> None:
        ...
        Raises:
            LHPFileError: If databricks.yml doesn't exist
        ...
        if not databricks_file.exists():
            raise create_missing_databricks_file_error(str(databricks_file))
        ...
        try:
            ...
            try:
                data = load_yaml_file(
                    databricks_file, allow_empty=False, error_context="databricks.yml"
                )
            except (ValueError, LHPError) as e:
            ...
            self.logger.debug(
                f"All required targets exist in databricks.yml: {environments}"
            )
    ...
    def _load_substitution_values_for_environment(
        ...
        try:
            ...
            self.logger.debug(
                f"Loaded values for {env}: {catalog_var}={catalog_value}, {schema_var}={schema_value}"
            )

            return {"catalog": catalog_value, "schema": schema_value}

        except LHPError as e:
            self.logger.warning(
                f"Could not load substitution values from {substitution_file}: {e.title}"
            )
            return {"catalog": "main", "schema": f"lhp_${{bundle.target}}"}
        except Exception as e:
            self.logger.warning(
                f"Unexpected error loading substitution values from {substitution_file}: {e}"
            )
            return {"catalog": "main", "schema": f"lhp_${{bundle.target}}"}

    def _extract_database_from_python_files(
        self, pipeline_name: str, output_dir: Union[Path, str]
    ) -> Dict[str, str]:
        """
        ...
        for py_file in python_files:
            try:
                ...
                processed_files += 1
                self.logger.debug(
                    f"Extracted {len(file_database_values)} database values from {py_file.name}"
                )
            except Exception as e:
                self.logger.debug(f"Could not read {py_file}: {e}")
                continue
        ...
        self.logger.debug(
            f"Processed {processed_files} Python files with {len(database_values)} total database values"
        )
        ...
        if not most_common_db:
            self.logger.info(
                f"No database values found in Python files for pipeline '{pipeline_name}', using defaults"
            )
            return {"catalog": "main", "schema": f"lhp_${{bundle.target}}"}
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if "." in match and len(match.split(".")) >= 2:
                    ...
                    self.logger.debug(
                        f"Found database pattern: {database_value} from table: {match}"
                    )
    ...
    def _parse_resolved_database_string(self, database_string: str) -> Dict[str, str]:
        ...
        if not database_string or "." not in database_string:
            self.logger.debug(
                f"Invalid database string '{database_string}', using defaults"
            )
            return {"catalog": "main", "schema": f"lhp_${{bundle.target}}"}

        # Split on first dot only (catalog.schema)
        parts = database_string.split(".", 1)
        result = {"catalog": parts[0], "schema": parts[1]}

        self.logger.debug(
            f"Parsed resolved database string '{database_string}' to: {result}"
        )
        return result
    ...
    def _handle_pipeline_error(
        ...
        if isinstance(error, YAMLParsingError):
            error_msg = (
                f"YAML processing failed for pipeline '{pipeline_name}': {error}"
            )
    ...
    def _setup_sync_environment(
        self, env: str, output_dir: Path
    ) -> tuple[List[Path], set[str], List[Dict[str, Any]]]:
        """
        ...
        current_pipeline_names = {
            pipeline_dir.name for pipeline_dir in current_pipeline_dirs
        }
    ...
    def _process_current_pipelines(
        ...
        for pipeline_dir in current_pipeline_dirs:
            ...
            try:
                ...
                raise  # Let structured LHPErrors propagate to CLI error boundary
            except (YAMLParsingError, OSError) as e:
    ...
    def _log_sync_summary(self, updated_count: int, removed_count: int) -> None:
        ...
        if updated_count > 0 or removed_count > 0:
            if updated_count > 0 and removed_count > 0:
                self.logger.info(
                    f"Bundle sync completed: updated {updated_count}, deleted {removed_count} resource file(s)"
                )
            elif updated_count > 0:
                self.logger.info(
                    f"Updated {updated_count} bundle resource file(s) (created new or replaced user files)"
                )
            else:
                self.logger.info(
                    f"Deleted {removed_count} orphaned bundle resource file(s)"
                )
        else:
            self.logger.info(
                "All bundle resources preserved (conservative approach - existing LHP files untouched)"
            )
    ...
    def _is_lhp_generated_file(self, resource_file: Path) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"Could not read file {resource_file} for LHP detection: {e}"
            )
            return False
        ...
        try:
            ...
            for resource_file in self.resources_dir.glob("*.yml"):
                ...
                if pipeline_name:
                    ...
                    self.logger.debug(
                        f"Found resource file: {resource_file.name} for pipeline: {pipeline_name}"
                    )
    ...
    def _extract_pipeline_name_from_filename(
        self, resource_file: Path
    ) -> Optional[str]:
        """
    ...
    def _delete_resource_file(self, resource_file: Path, pipeline_name: str):
        ...
        try:
            if resource_file.exists():
                ...
                self.logger.info(
                    f"Deleted resource file: {resource_file.name} (pipeline '{pipeline_name}' no longer exists)"
                )
            else:
            ...
            raise BundleResourceError(
                f"Failed to delete resource file {resource_file}: {e}", e
            )
    ...
    def _backup_single_file(self, resource_file: Path, pipeline_name: str):
        ...
        try:
            if resource_file.exists():
                ...
                self.logger.info(
                    f"Backed up file: {resource_file.name} -> {backup_path.name}"
                )
