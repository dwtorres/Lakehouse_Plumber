...
class StateAnalyzer:
    ...
    def find_stale_files(
        ...
        if global_deps_changed:
            ...
            self.logger.debug(
                f"Global dependencies changed for {environment} - marking all files as stale"
            )
    ...
    def check_global_dependencies_changed(
        ...
        try:
            ...
            current_global_deps = self.dependency_resolver.resolve_global_dependencies(
                environment
            )
            ...
            config_changed = self.dependency_changed(
                stored_global_deps.project_config, current_global_deps.get("lhp.yaml")
            )
            ...
            self.logger.warning(
                f"Failed to check global dependencies for {environment}: {e}"
            )
            # Assume changed to be safe
            return True

    def check_file_dependencies_changed(
        self, file_state: FileState, environment: str
    ) -> bool:
        """
        Check if file-specific dependencies have changed.
        ...
        try:
            ...
            if set(current_deps.keys()) != set(stored_deps.keys()):
                self.logger.debug(
                    f"File dependency set changed for {file_state.generated_path}"
                )
                return True
            ...
            for dep_path, current_dep in current_deps.items():
                ...
                if self.dependency_changed(stored_dep, current_dep):
                    self.logger.debug(
                        f"File dependency {dep_path} changed for {file_state.generated_path}"
                    )
                    return True
            ...
            self.logger.warning(
                f"Failed to check file dependencies for {file_state.generated_path}: {e}"
            )
            # Assume changed to be safe
            return True

    def dependency_changed(
        self,
        stored_dep: Optional[DependencyInfo],
        current_dep: Optional[DependencyInfo],
    ) -> bool:
        """
    ...
    def get_files_needing_generation(
        ...
        stale_files = self.find_stale_files(
    ...
    def compare_with_current_state(
        ...
        if pipeline:
                ...
                try:
                    ...
                    for fg in flowgroups:
                        if fg.pipeline == pipeline:
                            ...
                            self.logger.debug(
                                f"Found YAML file for pipeline '{pipeline}': {yaml_file}"
                            )
            ...
            self.logger.debug(
                f"Found {len(current_yamls)} YAML file(s) for pipeline '{pipeline}'"
            )
    ...
    def find_new_yaml_files(
        ...
        if new_files:
            self.logger.info(
                f"Found {len(new_files)} new YAML file(s) for {environment}"
            )
            for f in new_files:
    ...
    def get_current_yaml_files(
        ...
        if include_patterns:
            ...
            filtered_by_patterns = discover_files_with_patterns(
    ...
    def calculate_expected_files(
        self, output_dir: Path, env: Optional[str] = None, discoverer=None
    ) -> Set[Path]:
        """
        ...
        try:
            ...
            for flowgroup in all_flowgroups:
                ...
                expected_file = (
                    output_dir / flowgroup.pipeline / f"{flowgroup.flowgroup}.py"
                )
                expected_files.add(expected_file.resolve())

                self.logger.debug(
                    f"Expected file: {expected_file} (from {flowgroup.pipeline}/{flowgroup.flowgroup})"
                )
        ...
        if env:
            self.logger.debug(
                f"Calculated {len(expected_files)} expected files for environment '{env}'"
            )

        return expected_files

    def get_detailed_staleness_info(
        self, state: ProjectState, environment: str
    ) -> Dict[str, Any]:
        """
        ...
        if global_deps_changed:
            ...
            try:
                current_global_deps = (
                    self.dependency_resolver.resolve_global_dependencies(environment)
                )
                stored_global_deps = (
                    state.global_dependencies.get(environment)
                    if state.global_dependencies
                    else None
                )

                if not stored_global_deps:
                    if f"substitutions/{environment}.yaml" in current_global_deps:
                        result["global_changes"].append(
                            f"Substitution file (substitutions/{environment}.yaml) added"
                        )
                    if "lhp.yaml" in current_global_deps:
                        result["global_changes"].append(
                            "Project config (lhp.yaml) added"
                        )
                else:
                    ...
                    if self.dependency_changed(
                        ...
                        result["global_changes"].append(
                            f"Substitution file (substitutions/{environment}.yaml) changed"
                        )
                    ...
                    if self.dependency_changed(
                        ...
                        result["global_changes"].append(
                            "Project config (lhp.yaml) changed"
                        )

            except Exception as e:
                self.logger.warning(f"Failed to analyze global dependency changes: {e}")
                result["global_changes"].append(
                    "Global dependencies changed (details unavailable)"
                )
    ...
    def get_file_dependency_changes(
        self, file_state: FileState, environment: str
    ) -> List[str]:
        """
        ...
        try:
            ...
            if source_path.exists():
                ...
                if not file_state.source_yaml_checksum:
                    changes.append(
                        f"Source YAML checksum missing: {file_state.source_yaml}"
                    )
            ...
            self.logger.warning(
                f"Failed to analyze file dependency changes for {file_state.generated_path}: {e}"
            )
