"""
ADO Test Reporter (Config Mapping) — Publishes DQ test results to Azure DevOps Test Plans.

Uses a test_case_mapping in the config YAML to translate pipeline test_id values
(e.g., "SIT-G01") into ADO Test Case IDs (e.g., 2272983).
...
logger = logging.getLogger("ado_test_reporter")
...
def _resolve_results(results, test_case_mapping, tc_to_point, suite_id, context):
    """Resolve pipeline results to ADO result entries via test_case_mapping.
    ...
    for r in results:
        ...
        ado_tc_id = test_case_mapping.get(test_id)

        if not ado_tc_id or ado_tc_id == 0 or str(ado_tc_id) == "0000000":
            logger.warning(f"No valid ADO mapping for test_id='{test_id}' — skipping")
            failed_count += 1
            continue

        ado_tc_id = int(ado_tc_id)
        point_id = tc_to_point.get(ado_tc_id)
        if point_id is None:
            logger.warning(
                f"Test case {ado_tc_id} (test_id='{test_id}') has no test point "
                f"in suite {suite_id} — skipping"
            )
            failed_count += 1
            continue
        ...
        comment = (
            f"Test: {test_id} / {r.get('expectation_name', '')}\n"
...
def publish_results(results, config, context, spark):
    """Publish DQ test results to Azure DevOps Test Plans.
    ...
    terminal_state = context.get("terminal_state", "unknown")
    logger.info(
        f"ADO reporter invoked — {len(results)} result(s), "
    ...
    if config.get("dry_run", False):
        ...
        for r in results:
            logger.debug(
                f"  {r.get('test_id')}/{r.get('expectation_name')}: {r.get('status')}"
            )
    ...
    try:
        ...
        test_case_mapping = config.get("test_case_mapping", {})

        # Step 1: GET test points for the suite
        tc_to_point = _get_test_points(base, headers, plan_id, suite_id, api_version)

        # Step 1b: Resolve results via mapping
        matched_point_ids, result_entries, failed_count = _resolve_results(
            results, test_case_mapping, tc_to_point, suite_id, context
        )
