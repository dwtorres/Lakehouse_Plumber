...
""")
            ...
            (project_root / "pipelines" / "01_raw_ingestion" / "csv_ingestions").mkdir(
                parents=True
            )
            (project_root / "pipelines" / "01_raw_ingestion" / "json_ingestions").mkdir(
                parents=True
            )
    ...
    def test_cli_pipeline_flag_finds_by_field(
        self, runner, temp_project_with_pipeline_field_test
    ):
        ...
        try:
            ...
            generated_dir = (
                temp_project_with_pipeline_field_test
                / "generated"
                / "dev"
                / "raw_ingestions"
            )
    ...
    def test_cli_pipeline_flag_different_pipeline_fields(
        self, runner, temp_project_with_pipeline_field_test
    ):
    ...
    def test_validate_pipeline_flag_by_field(
        self, runner, temp_project_with_pipeline_field_test
    ):
