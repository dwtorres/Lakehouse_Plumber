"""Structural tests for the lhp_watermark package.
...
def test_watermark_manager_is_a_package() -> None:
    ...
    spec = importlib.util.find_spec("lhp_watermark")
    assert spec is not None, "lhp_watermark is not importable"
    assert spec.submodule_search_locations is not None, (
        "lhp_watermark is not a package "
...
def test_manager_submodule_holds_class_body() -> None:
    ...
    submodule = importlib.import_module("lhp_watermark.watermark_manager")
    assert hasattr(
...
def test_public_watermark_manager_reexport_identity() -> None:
    ...
    public = importlib.import_module("lhp_watermark")
    internal = importlib.import_module("lhp_watermark.watermark_manager")
...
def test_public_import_surface_unchanged() -> None:
    ...
    from lhp_watermark import WatermarkManager  # noqa: F401
