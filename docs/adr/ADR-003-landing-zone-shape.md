...
- [ ] Q1 answered by ADR-001 author (rationale captured in this doc). **Deferrable** — underscore-prefix rationale is low-stakes and does not gate production work.
- [ ] Q2 retention policy ratified with operator contract + enforcement mechanism + benchmark at ≥10k subdirs. Phase D of follow-ups plan.
- [x] Q3 empty-batch case closed 2026-04-20. Two converging artefacts: (1) **code shipped** 2026-04-19 (`08fe67a1` — extractor template writes coalesced 0-row schema-bearing parquet from live `df.schema` when JDBC returns zero rows; `landing_empty_schema_fallback` log event for observability); (2) **live validation** 2026-04-20 against devtest workspace `dbc-8e058692-373e` per [`docs/runbooks/devtest-validation-adr-003-phase-a-c.md`](../runbooks/devtest-validation-adr-003-phase-a-c.md) §Step 4 — sentinel watermark at `2099-12-31` forced an empty incremental on `pg_supabase_aw / HumanResources / Department`; extractor wrote run-scoped parquet at `_lhp_runs/local-f7816a34-ec4c-494c-acdc-252262acb030/` containing one part file with 0 rows + full Department schema; bronze AutoLoader did not raise `CF_EMPTY_DIR_FOR_SCHEMA_INFERENCE`. Watermark row written with `row_count=0`, `status=completed`, `watermark_value=2099-12-31T00:00:00.000000+00:00`.
- [~] A second batch-loader source type. **Deferred** 2026-04-19 — fork has no API-ingestion consumer; building `api_watermark_v2` solely to satisfy this success criterion is speculation. Q4 disposition: codebase remains on Alternative A (status quo, `_lhp_runs/{run_id}/` + Path ③ glob); revisit when a real second-source consumer materializes. Cold-start implementation plan preserved in [`docs/planning/b1-api-watermark-blueprint.md`](../planning/b1-api-watermark-blueprint.md).
- [x] Q5 closed 2026-04-19. Three converging artefacts: (1) Wumbo PR #2 — dedicated landing schema sidesteps UC `LOCATION_OVERLAP`; (2) `49fe740a` — generator-side enforcement (`LHPConfigError LHP-CFG-018`) on landing-path overlap with bronze write_target; (3) [`Example_Projects/edp_lhp_starter/`](../../Example_Projects/edp_lhp_starter/) — canonical `<env>_edp_<medallion>` layout with external-ADLS-backed UC volumes, validated by 5 e2e tests across devtest/qa/prod; (4) [ADR-004](ADR-004-watermark-registry-placement.md) — per-env watermark registry placement (Option B). External ADLS volume provisioning is platform-team responsibility outside this fork.
- [~] Final decision between A/B/C/D. **Provisionally Alternative A (status quo)** on JDBC-only evidence. Formal close awaits a second-source consumer or explicit decision to abandon multi-source generalization as a closure requirement. No migration pending.
