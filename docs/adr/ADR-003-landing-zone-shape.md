...
- [~] Q3 empty-batch case verified end-to-end. **Code shipped** 2026-04-19 (`08fe67a1` — extractor template writes coalesced 0-row schema-bearing parquet from live `df.schema` when JDBC returns zero rows; `landing_empty_schema_fallback` log event for observability). Wumbo dev-workspace `WHERE 1=0` validation outstanding.
