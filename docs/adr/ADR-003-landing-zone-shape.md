...
- [~] Q5 partially answered by Wumbo PR #2 (2026-04-19): dedicated landing schema sidesteps UC `LOCATION_OVERLAP`. **Generator-side enforcement shipped** 2026-04-19 (`49fe740a` — `LHPConfigError LHP-CFG-018` raised when `landing_path` resolves into the same UC catalog/schema as the sibling bronze `write_target`; warning emitted on cross-catalog landing). Production external-ADLS-volume pattern still open (Phase C of follow-ups plan).
