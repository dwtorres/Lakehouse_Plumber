...
|---|---|---|---|---|---|---|---|
| devtest | yes | yes (no-op; empty registry) | yes (no-op; numFiles 0→0) | yes (V1-V5 all PASS) | 2026-04-25 | preflight stmt 01f1410e-a704; init via `lhp init-registry`; backfill+optimize via `lhp tier2-rollout`; validate Job run 125858824390202 (task run 323155701139961). Principal: `verbena1@gmail.com`. Warehouse: `9f30611e0b932a47`. | Empty registry (numFiles=0, sizeInBytes=0). M14 idempotency confirmed (re-run init issued zero ALTERs). Phase 5 ran on serverless notebook compute via new `lhp validate-tier2 --serverless` (no cluster needed); `lhp_watermark/` vendored via ADR-002 path (`lhp sync-runtime` + `bundle deploy`). V1=PASS V2=PASS V3=PASS V4=PASS V5=PASS, all under 18s each. Phase 6 dry-run validated for `customer_bronze` (zero-row preview = correct cold-start). Bugs fixed mid-flight: SDK 0.105 needs enum `ExecuteStatementRequestOnWaitTimeout.CONTINUE` not string; framing correction — vendoring path per ADR-002 not wheel-attach. Two CLI bugs surfaced for backlog: (a) `_wait_for_run` `get_run_output(run_id=…)` errors when job retried (multi-task); should resolve task-level run_id from `get_run(run_id).tasks[*].run_id`. (b) `--probe-schema` cross-namespace mode breaks notebook (`DEEP CLONE source.watermarks` resolves with widget catalog/schema, so probe-schema-different-from-registry-schema causes `TABLE_OR_VIEW_NOT_FOUND`). Workaround: pass `--probe-schema metadata.devtest_orchestration` (same as registry). |
| qa  | … | … | … | … | … | … | … |
| prod | … | … | … | … | … | … | … |
